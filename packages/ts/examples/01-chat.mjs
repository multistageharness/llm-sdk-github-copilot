...
//
// Run:
//   node examples/01-chat.mjs [--cli-path /path/to/copilot] [--proxy http://user:pass@host:8080]
...
await runExample(harness, async () => {
  const prompt = args._[0] ?? 'In one sentence: what does a token budget protect against?';
