...
import { CopilotHarness, createHarness, formatSSE, renderContextPreamble } from '../src/harness.mjs';
...
test('chat: returns content, tracks usage, fires run events', async () => {
  const { deps, ref } = mockDeps({ script: [{ content: 'four', inputTokens: 12, outputTokens: 3 }] });
  const harness = new CopilotHarness({}, deps);
  const events = [];
  harness.on('run:start', () => events.push('run:start'));
  harness.on('run:end', () => events.push('run:end'));
  harness.on('usage:tokens', (r) => events.push(`tokens:${r.inputTokens}/${r.outputTokens}`));
  ...
  assert.ok(events.includes('run:start') && events.includes('run:end') && events.includes('tokens:12/3'));
...
test('systemPromptMode replace and per-call model override', async () => {
  ...
  await harness.createSession({ model: 'claude-sonnet-4.6', reasoningEffort: 'high' });
  const cfg = ref.client.lastSessionConfig;
...
test('instruction document file becomes the system prompt', async () => {
  ...
  const { deps, ref } = mockDeps({ script: [{ content: 'ok' }] });
  const harness = new CopilotHarness({ config: { systemPromptFile: file } }, deps);
  await harness.chat('hi');
  assert.match(ref.client.lastSessionConfig.systemMessage.content, /Always cite sources/);
  await harness.stop();
});
...
test('HTTP proxy is scoped to the child env, not process.env', async () => {
  const { deps, ref } = mockDeps({ script: [{ content: 'ok' }] });
  const harness = new CopilotHarness({
    config: { httpProxy: 'http://user:pass@proxy:8080' },
  }, deps);
  await harness.start();
  assert.equal(ref.client.options.env.HTTPS_PROXY, 'http://user:pass@proxy:8080');
  assert.equal(ref.client.options.env.HTTP_PROXY, 'http://user:pass@proxy:8080');
  assert.notEqual(process.env.HTTPS_PROXY, 'http://user:pass@proxy:8080');
  await harness.stop();
});
...
test('token budget blocks a run whose estimate exceeds the ceiling', async () => {
  ...
  harness.hooks.register('onBudgetExceeded', (p) => { exceededHook = p; });

  await assert.rejects(
    harness.chat('this prompt is far too long for a five token budget, clearly'),
...
test('enforcement=warn lets the run proceed and emits budget:warn', async () => {
  const { deps } = mockDeps({ script: [{ content: 'went through', inputTokens: 50, outputTokens: 10 }] });
  const harness = new CopilotHarness({
  ...
  const result = await harness.chat('long prompt that will not fit in ten tokens at all');
...
test('stream yields deltas then the final message', async () => {
  ...
  assert.ok(deltas.length >= 2, `expected multiple deltas, got ${deltas.length}`);
...
test('structured output: parses on first try', async () => {
  const { deps } = mockDeps({ script: [{ content: '{"name":"Ada","age":36}' }] });
  const harness = new CopilotHarness({}, deps);
  const schema = {
    type: 'object',
    required: ['name', 'age'],
  ...
  const { value, attempts } = await harness.structured('Extract the person', schema, {
...
test('structured output: repairs after an invalid reply', async () => {
  ...
  const { value, attempts, usage } = await harness.structured('Extract', schema);
  assert.deepEqual(value, { name: 'Ada', age: 36 });
  assert.equal(attempts, 2);
  assert.equal(invalidEvents.length, 1);
  assert.equal(usage.totalTokens, 30); // two mock calls at 15 each
  assert.match(ref.client.sessions[0].sent[1].prompt, /not valid against the required JSON Schema/);
  await harness.stop();
});
...
test('registered tools are wrapped: success, failure, and hook denial', async () => {
  ...
  harness.hooks.register('beforeToolCall', ({ args }) => (
    args?.q === 'secret' ? { deny: true, reason: 'q is classified' } : undefined
  ));
  ...
  assert.deepEqual(emitted, ['tool:call', 'tool:result', 'tool:denied', 'tool:call', 'tool:error']);
  await harness.stop();
});
...
test('SDK hook bridge: onPreToolUse deny + lifecycle hooks fire', async () => {
  ...
  harness.hooks.register('beforeToolCall', ({ toolName }) => (
    toolName === 'shell' ? { deny: true, reason: 'no shell allowed' } : undefined
  ));
  const seen = [];
  harness.hooks.register('afterToolCall', (p) => seen.push(['after', p.toolName]));
  harness.hooks.register('onPromptSubmit', (p) => seen.push(['prompt', p.prompt]));
  ...
  const deny = await sdkHooks.onPreToolUse({ toolName: 'shell', toolArgs: {}, sessionId: 's' });
  assert.equal(deny.permissionDecision, 'deny');
  assert.match(deny.permissionDecisionReason, /no shell/);

  const allow = await sdkHooks.onPreToolUse({ toolName: 'web', toolArgs: {}, sessionId: 's' });
  assert.equal(allow, undefined);

  await sdkHooks.onPostToolUse({ toolName: 'web', toolArgs: {}, toolResult: {}, sessionId: 's' });
  await sdkHooks.onUserPromptSubmitted({ prompt: 'p', sessionId: 's' });
  await sdkHooks.onSessionStart({ sessionId: 's', source: 'new' });
  await sdkHooks.onErrorOccurred({ error: 'x', errorContext: 'model_call', sessionId: 's' });

  assert.deepEqual(seen, [['after', 'web'], ['prompt', 'p'], ['start'], ['error', 'model_call']]);
  await harness.stop();
});
...
test('attachContext prepends prior conversation to the next prompt only', async () => {
  const { deps, ref } = mockDeps({ script: [{ content: 'a1' }, { content: 'a2' }] });
  const harness = new CopilotHarness({}, deps);
...
test('usageReport includes budget line when configured', async () => {
  const { deps } = mockDeps({ script: [{ content: 'ok', inputTokens: 30, outputTokens: 10 }] });
  const harness = new CopilotHarness({
...
test('renderContextPreamble formats roles and is empty for no messages', () => {
  ...
  const text = renderContextPreamble([{ role: 'user', content: 'q' }, { content: 'r' }]);
